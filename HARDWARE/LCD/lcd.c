...
//mode:叠加方式(1)还是非叠加方式(0)
//color:数字的颜色
void LCD_Fast_ShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode,u16 color)
{  							  
    u8 temp,t1,t;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数	
...
//num:数值(0~4294967295);	
//color:数字的颜色
void LCD_Fast_ShowNum(u16 x,u16 y,u32 num,u8 len,u8 size,u16 color)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_Fast_ShowChar(x+(size/2)*t,y,' ',size,0,color);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_Fast_ShowChar(x+(size/2)*t,y,temp+'0',size,0,color); 
	}
} 
//显示数字,高位为0,还是显示
//x,y:起点坐标
...
//[0]:0,非叠加显示;1,叠加显示.
//color:数字的颜色
void LCD_Fast_ShowxNum(u16 x,u16 y,u32 num,u8 len,u8 size,u8 mode,u16 color)
{  
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode&0X80)LCD_Fast_ShowChar(x+(size/2)*t,y,'0',size,mode&0X01,color);  
				else LCD_Fast_ShowChar(x+(size/2)*t,y,' ',size,mode&0X01,color);  
 				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_Fast_ShowChar(x+(size/2)*t,y,temp+'0',size,mode&0X01,color); 
	}
} 
//显示字符串
//x,y:起点坐标
...
void Printf(u16 x,u16 y,u8 size,char *format,...)
{
	...
	else
	{
		...
		do
		{
			sprintf(tempbuf+index,"%c",va_arg(ap,char));
			index++;
		...
		LCD_ShowString(x,y,120,16,size,format);
	}
